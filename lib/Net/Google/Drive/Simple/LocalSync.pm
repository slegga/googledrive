...
#	my $rffs = _get_rem_value($remote_file,'fileSize'); #object or hash
    ...
    my ($tmp_loc_md5_hex,$tmp_new_rem_md5)=($loc_md5_hex//'', $new_rem_md5//'xfghfghkdfngsl');
    if ( $tmp_loc_md5_hex eq $tmp_new_rem_md5 ||
    	($rem_mod > $loc_mod && $tmp_new_rem_md5 eq ($filedata->{rem_md5_hex}//'x')) ||
        ($rem_mod < $loc_mod && $tmp_loc_md5_hex eq ($filedata->{loc_md5_hex}//'x'))
    	) {
